...
export class BoxWrapperService {
     ...
     */
    static async detectTier(client, enterpriseId) {
        let tier = 'free';
        try {
            await client.metadataTemplates.getMetadataTemplate('enterprise', 'taxFlowClientProfile');
            ...
            if (statusCode === 404) {
                ...
                console.warn(`[BoxWrapperService] Tier detection failed for enterprise ${enterpriseId}, defaulting to free tier:`, error instanceof Error ? error.message : String(error));
                tier = 'free';
            }
        }
        console.info(`[BoxWrapperService] Detected tier: ${tier} for enterprise ${enterpriseId}`);
        return {
            tier,
            enterpriseId,
    ...
    async createAutomatedVault(clientName, externalId, email) {
        ...
        if (tier === 'free') {
            console.warn(`[BoxWrapperService] Free tier: skipping metadata and cascade policy for vault folder ${folderId}`);
            return {
                folder: {
                    id: folder.id,
                    name: folder.name,
        ...
        try {
            await client.folderMetadata.createFolderMetadataById(folderId, 'enterprise', 'taxFlowClientProfile', {
            ...
            throw new Error(`Failed to apply metadata to vault folder ${folderId}: ${error.message ?? error}`);
        }
        ...
        try {
            ...
            throw new Error(`Vault folder ${folderId} created but cascade policy failed: ${error.message ?? error}. Manual remediation required.`);
        }
    }
    /**
    ...
    async findVaultByExternalId(externalId) {
        ...
        if (tier === 'enterprise') {
            let result;
            try {
                result = await client.metadataQueries?.executeRead?.({
